...
module.exports.onLoad = async() => {
	...
	const path = resolve(__dirname, 'cache/canvas', 'frame5.jpeg');
	if (!existsSync(dirMaterial + "canvas")) mkdirSync(dirMaterial, { recursive: true });
	if (!existsSync(path)) await downloadFile("https://drive.google.com/uc?id=14H4mIR73kIYoGkjLk-yVPZIHYGpPZdiq", path);
}
...
async function makeImage({ one, two }) {
	...
	let batgiam_img = await jimp.read(__root + "/frame5.jpeg");
	...
	batgiam_img.composite(circleOne.resize(200, 200), 500, 90).composite(circleTwo.resize(150, 150), 108, 108);
